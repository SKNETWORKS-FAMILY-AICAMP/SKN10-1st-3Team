...
use car_data_analysis;

-- 테이블 목록을 확인
show tables;

-- vehicle_registration_data 테이블의 모든 데이터를 조회
select * from vehicle_registration_data;

-- formatted_date 컬럼을 DATE 타입으로 추가
ALTER TABLE vehicle_registration_data ADD COLUMN formatted_date DATE;
...
ALTER TABLE vehicle_registration_data ADD COLUMN year INT;
...
SELECT formatted_date, year FROM vehicle_registration_data;

-- 쉼표 제거: personal, commercial, total, year 컬럼의 값에서 쉼표(,)를 제거
UPDATE 
	vehicle_registration_data
SET 
...
-- 데이터 타입 변경: personal, commercial, total 컬럼을 INT 타입으로 변환
ALTER TABLE 
	vehicle_registration_data
MODIFY COLUMN 
	personal INT,
